...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="openai/clip-vit-large-patch14", device="cuda", with_bbox=False, max_length=77, extend_outputlen=None):
        super().__init__()
        ...
        if with_bbox: ## expand box vocab
            ...
            ofa_weight = torch.load('ReCo/dataset/ofa_base_enc_weight.pt').to(cliptext_weight.device)
            init_weight = torch.cat([cliptext_weight,ofa_weight[58457:]],dim=0)
            n, d = cliptext_weight.shape[0]+1000, cliptext_weight.shape[1]
            expand_word_emd = nn.Embedding(n, d, _weight=init_weight).to(cliptext_weight.device)
            self.transformer.text_model.embeddings.token_embedding = expand_word_emd
